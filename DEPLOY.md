...
**No role vocabulary is declared, deliberately.** The `admin` flag here reaches
`/admin/keys/{id}/tdm` — the TDM credentials of third-party institutions, given
for an agreed use. Saving one click at onboarding is not worth putting those
behind a header, so that flag stays a manual decision. `/runs` is a permanent
redirect to `/app` that carries its query string.
