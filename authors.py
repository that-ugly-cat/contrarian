"""Author-string handling (derived from LSSR's authors.py).
...
split_authors() also copes with legacy comma-joined strings; author_key()
merges the different forms of the same person. Used by the source adapters
(canonical ingest) and by the reference assembler (surname/initial extraction
for procedural citations).
"""
import re
...
def split_authors(raw: str | None) -> list[str]:
    ...
    for p in pieces:
        if authors and is_initials(p):     # 'Rossi' + 'M' → 'Rossi, M'
            ...
            authors.append(p)
    # 'Surname, Given, Surname, Given' lists: every piece a single full word
    # and the count is even → pair them up
